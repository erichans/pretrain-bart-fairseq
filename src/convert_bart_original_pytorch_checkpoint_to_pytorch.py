...
@torch.no_grad()
def convert_bart_checkpoint(checkpoint_dir, checkpoint_path, pytorch_dump_folder_path, hf_checkpoint_name=None):
    """
    Copy/paste/tweak model's weights to our BERT structure.
    """
    if os.path.exists(f'{checkpoint_dir}/{checkpoint_path}'):
        bart = BARTModel.from_pretrained(checkpoint_dir, checkpoint, bpe=None)
    else:
        bart = torch.hub.load(checkpoint_dir, checkpoint_path).eval()
    ...
    # TODO: Verificar como checar depois. O encoding deve ser diferente mesmo
    ...
    if checkpoint_path == "bart.large.mnli":
        ...
        # rename_key(state_dict, 'decoder.output_projection.weight', 'lm_head.weight')
        lm_head = state_dict.pop('decoder.output_projection.weight')
